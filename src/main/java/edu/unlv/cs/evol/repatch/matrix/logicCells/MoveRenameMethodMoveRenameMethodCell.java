...
import edu.unlv.cs.evol.repatch.utils.MatrixUtils;
...
public class MoveRenameMethodMoveRenameMethodCell {
    ...
    public boolean checkOverrideConflict(RefactoringObject dispatcherObject, RefactoringObject receiverObject) {
        ...
        if(psiReceiver != null && psiDispatcher != null) {
            if (!MatrixUtils.ifClassExtends(psiDispatcher, psiReceiver)) {
                return false;
            }
        }
        ...
        return !MatrixUtils.isSameName(dispatcherOriginalMethodName, receiverOriginalMethodName) &&
                MatrixUtils.isSameName(dispatcherNewMethodName, receiverNewMethodName) &&
